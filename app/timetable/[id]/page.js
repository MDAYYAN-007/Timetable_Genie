      ...
      try {
        ...
        if (session) {
          ...
          if (result.data && result.data.length > 0) {
            ...
            console.log("Sorted result data:", result.data);
            ...
            if (formData) {
              ...
              const res = await storeGeneratedTimetables(timetable, id, 1);
              const result = await getGeneratedTimetables(id);
              if (result.data && result.data.length > 0) {
                result.data.sort((a, b) => a.tt_no - b.tt_no);
                result.data.forEach((entry) => {
  ...
  const handleRegenerate = (tt_no) => async () => {
    ...
    try {
      if (session) {
        ...
        if (formData) {
          const timetable = await generateTimetable(formData.data);
          const res = await storeGeneratedTimetables(timetable, id, tt_no);
          const result = await getGeneratedTimetables(id);
          if (result.data && result.data.length > 0) {
            result.data.sort((a, b) => a.tt_no - b.tt_no);
            result.data.forEach((entry) => {
